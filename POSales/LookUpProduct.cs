...
namespace POSales
{
    public partial class LookUpProduct : Form
    {
        ...
        public void LoadProduct()
        {
            ...
            cm = new SqlCommand("SELECT p.pcode, p.barcode, p.pdesc, c.category, p.price, p.qty FROM tbProduct AS p INNER JOIN tbCategory AS c on c.id = p.cid WHERE CONCAT(p.pdesc, c.category) LIKE '%" + txtSearch.Text + "%'", cn);
            cn.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
